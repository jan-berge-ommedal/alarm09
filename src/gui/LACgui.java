...
public class LACgui extends JPanel implements Values, ActionListener, PropertyChangeListener {
	...
	*/
	...
	/**
	 * Denne metoden oppf�rer seg som en hjelpemetode til konstrukt�ren som genererer GUIet. Dersom
	 * den kalles med true vil den sette guiet til � f�lge en modell
	...
	private void initialize() {
		...
		if (model) { //hvis initialize kalles med en model settes listen til � v�re med elementene
			...
			try {
				...
				System.err.println("nullpointerex n�r LAC ID skulle settes: trace f�lger under");
				npe.printStackTrace();
	...
	/**
	 * Metode som kalles for � installere sensor
	 */
	public void installSensor(boolean makeRoom) {
		...
		if (!makeRoom) { //skal ikke lages nytt rom
			/*
			 * F�lgende er unike for tilfeller der rom ikke skal lages
			 */
	...
	/**
	 * �pner vindu slik at et rom kan editeres
	...
	/**
	 * indre klasse som tar for seg funksjonalitet rundt install sensor, n�r save trykkes
	...
	class SensorAttributesListener implements ActionListener {
		...
		/**
		 * konstrukt�r for saveknappen ved install sensor
		 * @param frame
		 * @param mec
		 * @param roomsList
		 */
